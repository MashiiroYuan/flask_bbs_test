from flask import Blueprint, views, render_template, request, make_response, session, url_for, g,abort
...
def index():
    ...
    query_obj=None
    # if sort==1:
    #     query_obj=PostModel.query.order_by(PostModel.desc())
    # elif sort==2:
    #     #精华倒叙
    ...
    total = 0
    # if board_id:
    #     posts = query_obj.filter_by(board_id=board_id).slice(start, end)
...
def add_comment():
    ...
    if form.validate():
        ...
        if post:
            ...
            return restful.params_error('没有这帖子')
    else:
